...
如果你的输入分辨率小于 640x640，YOLO 会执行 **放大 (Upscaling)** 操作。

- **无信息丢失**：原始 320x120 图片中的每个像素都会被保留。实际上，每个原始像素在模型视野中都会变成一个更大的像素块。
- **提高精度**：对于姿态识别 (Pose) 模型，放大图像有助于网络更清晰地“看到”眼睛、手指等细小特征。
- **逻辑一致性**：即使填充了大量的灰色边框，物体的几何形状依然 100% 保持正确，不会产生任何扭曲。
