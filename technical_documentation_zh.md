...
- **实时视频流**：基于 OpenCV 的脚本，提供低延迟的实时姿态估计，并支持动态阈值调节。
...
### 2.1 模型规范
- **模型文件**：`yolo11n-pose.pt`（Nano 版本，兼顾速度与精度）。
- **任务类型**：姿态估计（关键点检测）。
- **输出内容**：人体 17 个关键点（joints）及其置信度分数，以及人体的检测框（Bounding Box）。
...
- **引擎**：OpenCV (cv2)。
- **图像格式**：NumPy 数组（BGR 格式）。
- **实时控制**：集成 OpenCV Trackbars，允许在视频运行中动态微调参数。
- **性能优化**：采用 `predict()` 模式替代 `track()`，以减少对第三方库（如 `lap`）的依赖，确保高帧率（FPS）。
